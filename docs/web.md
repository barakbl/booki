...
| 📄 **Documents** | Scoped search over PDFs, ebooks, office files, plain-text formats. Toggle list ↔ grid. Per-extension icons. The matching is `kind == "document"` OR `"document" in sources`, so any item the `document` enricher (or another plugin) tags shows up here. |
